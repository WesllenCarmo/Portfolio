...
const Home = () => {
  ...
  return (
    <div>
        ...
        <div className='flex flex-col gap-4 w-fit items-center lg:items-start'>
          ...
          <div className='flex flex-col lg:flex-row gap-8 justify-between items-center flex-wrap md:gap-8'>
            <a href='/#contact' aria-label={t("hero.contactButton.label")} className='bg-custom-light-blue border-4 border-custom-light-blue text-custom-dark-blue text-3xl font-bold px-10 py-2 rounded-xl transition duration-400 hover:bg-custom-dark-blue hover:text-custom-light-blue'>
              {t("hero.contactButton.text")}
            </a>
            <a href={Resume} download aria-label={t("hero.resumeButton.label")} className='border-custom-light-blue border-4 text-3xl text-custom-light-blue font-bold px-6 py-1 rounded-xl hover:bg-custom-light-blue hover:text-custom-dark-blue transition duration-400'>
              {t("hero.resumeButton.text")}
            </a>
          </div>
        </div>
        <img
      ...
      <main role='main' className='flex flex-col gap-16 mb-8'>
        <About />
        <Skills />
        <Projects />
        <Contact />
      </main>
      <footer role='contentinfo' className='border-t-2 px-8 py-4 border-custom-light-blue flex justify-between'>
        <a href='#home'>
          <img
            src={WLogo}
            alt={t("hero.logoLabel")}
            className='w-30'
          />
        </a>
        <div className='flex gap-8'>
          <a href='https://github.com/WesllenCarmo' target='_blank' aria-label={t("footer.githubLabel")}><FaGithub className='text-3xl text-custom-white' /></a>
          <a href='https://www.linkedin.com/in/wesllen-araujo0907/' target='_blank' aria-label={t('footer.linkedinLabel')}><FaLinkedin className='text-3xl text-custom-white' /></a>
          <a href='mailto:wesllenaraujo.developer@gmail.com' target='_blank' aria-label={t("footer.emailLabel")}><SiGmail className='text-3xl text-custom-white' /></a>
        </div>
      </footer>
      <BackToTopButton />
    </div>
